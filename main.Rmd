...
```{r}
cv.error.ST.kmin = c(2:50) %>% map_dfr(function(k_min) {c(`Valore parametro k_min` = k_min, `Errore` = mean(cv.error(Class ~ ., tree, data, nrow(data), minsize = k_min)))}) %>% ggplot(aes(x=`Valore parametro k_min`, y=`Errore`)) + geom_line() + ylim(0, 1)
cv.error.ST.kmin
```
...
LOOCV tree bagging
```{r}
mean(cv.error(Class ~ ., randomForest, data, nrow(data), mtry = ncol(data) - 1, ntree = optimal.TB.B)) # or could leave default
```

LOOCV RandomForest
```{r}
mean(cv.error(Class ~ ., randomForest, data, nrow(data), ntree = optimal.RF.B, mtry = optimal.RF.m))
```
