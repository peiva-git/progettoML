...
```{r}
results = expand_grid(kernel=c("linear","polynomial","radial","sigmoid"), cost=exp(seq(-6,8,1))) %>% rowwise() %>% mutate(error = mean(cv.error(isVersicolor~., svm, d, 10, kernel=kernel, cost=cost, degree=2)))
