...
```{r}
data = read.csv("/home/koso/Documenti/leaf.csv")
```
...
build tree
```{r}
singleTree = tree(Class ~ . - SpecimenNumber, data)
```
```{r}
plot(singleTree)
text(singleTree)
...
```{r}
cv.error(Class ~ . - SpecimenNumber, tree , data, 5)
```
...
We can use the cv.error function (k = 10)

```{r}
cv.error(formula = Class ~ . - SpecimenNumber, svm, data, 10, kernel="polynomial", degree=2)
```
