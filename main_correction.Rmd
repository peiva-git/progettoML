...
Error with ST model
```{r}
error.ST = cv.error.external.ST(Class ~ ., data, nrow(data))
```
...
Error with TB model
```{r}
error.TB = cv.error.external.TB(Class ~ ., data, nrow(data))
```

RF internal OOB
```{r}
internalOOB.RF = function(formula, data) {
  cv.error.RF.B.m = c(1:1000) %>% map_dfr(function(b) {
    c(1:15) %>% mapd_dfr(function(m) {
...
Error with RF model
```{r}
error.RF = cv.error.external.RF(Class ~ ., data, nrow(data))
```
...
internalCV.SVM = function(formula, data, k) {
  cv.error.SVM.C.kernel = seq(exp(-10), exp(10), 0.5) %>% map_dfr(function(c) {
    c("linear", "polynomial", "radial", "sigmoid") %>% mapd_dfr(function(ker) {
...
```{r}
error.SVM = cv.error.external.SVM(Class ~ ., data, nrow(data))
```
