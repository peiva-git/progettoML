...
Decision tree without parameters evaluation (Neast CV)
```{r}
...
Tree bagging without parameters evaluation (Neast CV)
```{r}
...
Random Forest without parameters evaluation (NeasT CV) 
```{r}
...
10-CV Naive Bayes 
```{r}
error.NB = mean(cv.error(Class ~ ., naiveBayes, data, 10))
error.NB                
```
...
error.RC = function(indexes){  
  for(i in c(1:nrow(indexes))) {
...
- RANDOM CLASSIFIER
```{r}
error.RC.default = mean(cv.error(Class ~ ., naiveBayes, data, 10))
error.RC.default               
```
